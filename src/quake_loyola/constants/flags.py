...
# (KNOTT_ENABLED, MARYLAND_ENABLED) default to False so only the world-shell
...
# master may themselves default to True so that flipping just the module
# master on reveals a fully-detailed module rather than an empty shell. The
# bridge (BRIDGE_ENABLED_SPAN_*), west campus (WEST_CAMPUS_ENABLED_*),
# entities.py (ENTITIES_ENABLED_*), and light groups (LIGHTS_ENABLED_*)
# sections instead use plain independent per-section flags with no overall
# master — see config.DEFAULTS for the authoritative per-flag default.
# ════════════════════════════════════════════════════════════════════════════════
